...
class BSI_excel_data < Source_data
 ...
 def initialize(filepath)
   ...
   worksheet = workbook.Worksheets(1)
     ...
     line = '2'
   progress = ProgressBar.new("Importing", 29000)
...
writeInfoPage(newItems.keys, "differences--#{oldFile.split(/\.| /)[2]}_#{newFile.split(/\.| /) [2]}.html")
writeToExcel(prepareOutput(newItems),  "differences--#{oldFile.split(/\.| /)[2]}_#{newFile.split(/\.| /) [2]}.xls")
